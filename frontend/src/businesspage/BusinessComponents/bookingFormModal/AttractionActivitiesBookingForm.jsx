...
const AttractionActivitiesBookingForm = ({ isOpen, onClose, product }) => {
  const [formData, setFormData] = useState({
    ...
    agreeToTerms: false,
    checkOutTime: new Date() // Default to current time or set a specific time
  });
  ...
  const steps = [
    ...
    <div key="step2" className="space-y-4">
      ...
      <Input type="date" label="Date of Visit" required fullWidth value={formData.visitDate} onChange={(e) => setFormData({ ...formData, visitDate: e.target.value })} />
      <TimeInput
        label="Preferred Time for Activities"
        value={formData.activityTime}
        onChange={(time) => setFormData({ ...formData, activityTime: time })}
        className="mb-4"
      />
      <TimeInput
    ...
    <div key="step3" className="space-y-4">
      ...
      <div className="mt-4">
        ...
        <p><strong>Activity Time:</strong> {formData.activityTime.toString()}</p>
